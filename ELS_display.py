...
class JDx_Display_Window(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.compass_le = self.findChild(QtWidgets.QLineEdit, "compass_le")
        self.log_filepath_le = self.findChild(QtWidgets.QLineEdit, "log_filepath_le")

        date_time = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        filepath = os.path.join(os.path.expanduser("~"), f"ELS_log_{date_time}.txt")
        ...
        pen_x = pyqtgraph.mkPen(color=(12, 145, 3))
        pen_y = pyqtgraph.mkPen(color=(31, 38, 237))
    ...
    def update_plot(self):
        ...
        send_serial_data(self.sensor, "*9900XY\r\n")
        data = read_serial_data(self.sensor).replace("+", "").replace("$", "")
        read_serial_data(self.sensor)  # read the echoed command.
        with open(self.log_filepath_le.text(), "a") as file:
            date_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            file.write(f"{date_time} - {data}")
        ...
        self.compass_le.setText(f"{compass}")
